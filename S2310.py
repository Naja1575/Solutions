...
class Product(Base):
    ...
    def __repr__(self):
        return f"Person({self.id=}    {self.product_number=}    {self.price=}    {self.brand=})"
...
def create_test_data():  # Optional. Used to test data base functions before gui is ready.
    with Session(engine) as session:
        new_items = []
        new_items.append(Customer(name="peter", address="Longstret 15", age=18))
        new_items.append(Customer(name="Hanna", address="Shortstret 62", age=35))
        new_items.append(Customer(name="Jane", address="Mediumstret 7", age=29))
        new_items.append(Product(product_number=5908536, price=275, brand="Arla"))
        new_items.append(Product(product_number=2953795, price=1500, brand="Nike"))
        new_items.append(Product(product_number=9563674, price=550, brand="Ikea"))
